...
class TimeSeries(Dataset):
    def __init__(self, df, mean, std, history, horizon):
        super().__init__()
        self.data, self.target, self.time, self.weekday = df_to_io(df, mean, std)
        self.history = history
        self.horizon = horizon
        self.days = self.data.size(0)
        self.daily_samples = tensor.size(1) - horizon
    ...
    def __getitem__(self, index):
        ...
        time = self.time[day, t:t+history]
        weekday = self.weekday[day, t]
        target = self.target[day, t+self.history:t+self.horizon]
        return data, target, time, day


def df_to_io(df, mean, std):
    # extract data

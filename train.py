...
inputs_train, inputs_valid, _ = Data.split_dataset(inputs, unit)
targets_train, targets_valid, _ = Data.split_dataset(targets, unit)

dataset = TensorDataset(inputs_train, targets_train)
dataloader = DataLoader(dataset, batch_size=args.bsz, shuffle=True)
inputs_valid = Utils.tensor2VarRNN(inputs_valid)
targets_valid = Utils.tensor2VarRNN(inputs_valid)
targets_valid_raw = targets_valid * data_std + data_mean
...
for epoch in range(args.nepoch):
    ...
    wape = Loss.WAPE(outputs, targets_valid_raw).data[0]
    mape = Loss.MAPE(outputs, targets_valid_raw).data[0]
